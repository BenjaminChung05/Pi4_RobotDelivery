...
class StateMachineConfig:
    symbol_cooldown_seconds: float = 1.0
...
class RobotStateMachine:
    ...
    def mark_output_symbol(self, label: str, now: float, cooldown_seconds: float | None = None) -> None:
        self.current_symbol = None
        self.last_detected_symbol = label
        cd = cooldown_seconds if cooldown_seconds is not None else self.config.symbol_cooldown_seconds
